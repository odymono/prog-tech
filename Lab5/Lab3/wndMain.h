...
//extern "C" int GraphColoring(int(*)[6]);

extern "C" int GC(int(*)[6], int numVertices, int*);
...
*/
int countVertices(const int matrix[6][6]) {
	int numVertices = 0;
	...
	case WM_COMMAND:
		...
		case OnButtonClicked:
			...
			if (ValidateMatrix()) {
				...
				int numVertices = countVertices(matrix);
				int result;
				if (numVertices == 0) {
					result = 0;
				}
				else {
					result = GC(matrix, numVertices, colors);
				};
				//int result = GraphColoring(matrix);
				path = "\nThe minimum number of colors needed is " + std::to_string(result) + "\n";
